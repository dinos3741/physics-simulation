   ...
   void display(Graphics g) {
    ...
    void checkEdges() {
        if (position.x - radius < 0) {
        	velocity.x *= -0.9;  // A little dampening when hitting the side
            position.x = radius;
        }
        ...
        if (position.y - radius < 0) {
            velocity.y *= -0.9;  // A little dampening when hitting the side
            position.y = radius;
        }
